...
function assemblesOutputFile(listWords) {
  ...
  listWords.forEach((paragraph, i) => {
    ...
    if (duplicates == "") return; // ? verificação para pular parágrafos sem textos duplicados

    finalText += `Palavras duplicadas no parágrafo ${i + 1}: ${duplicates} \n`;
  });

  return finalText;
}

export { assemblesOutputFile };
